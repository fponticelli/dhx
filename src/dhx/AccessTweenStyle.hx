...
class AccessTweenStyle<That : BaseTransition<Dynamic>> extends AccessTween<That>
{
...
#if thx
	public function colorNodef(f : HtmlDom -> Int -> thx.color.Rgb, ?priority : String)
	{
	...
	public function colorTweenNodef(tween : HtmlDom -> Int -> thx.color.Rgb -> (Float -> thx.color.Rgb), ?priority : String)
	{
		if (null == priority)
			priority = null; // FF fix
		var name = this.name;
