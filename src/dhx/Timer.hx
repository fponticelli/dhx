...
class Timer
{
	...
	public static function timer(f : Float -> Bool, ?delay : Float = 0.0)
	{
		...
		if (0 == interval)
		{
			dhx.Timer.timeout = untyped __js__("clearTimeout")(dhx.Timer.timeout);
			dhx.Timer.interval = 1;
			untyped window.requestAnimationFrame(dhx.Timer._step);
		}
	}
	...
	static function step()
	{
		...
		if (delay > 24)
		{
			if (Math.isFinite(delay))
			{
				untyped __js__("clearTimeout")(dhx.Timer.timeout);
				dhx.Timer.timeout = untyped __js__("setTimeout")(dhx.Timer._step, delay);
			}
			interval = 0;
		} else {
			interval = 1;
