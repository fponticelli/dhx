...
d3.Example.prototype = {
	...
	,run: function() {
		...
		dhx.Dom.select("#description").html().string(null != des?des:"");
		this.container = dhx.Dom.select(this.cid).attr("class").string(Type.getClassName(Type.getClass(this)).split(".").pop());
		this.runExample();
	}
	,runExample: function() {
		haxe.Log.trace("abstract method: " + Type.getClassName(Type.getClass(this)) + ".runExample",{ fileName : "Example.hx", lineNumber : 27, className : "d3.Example", methodName : "runExample"});
	}
	,destroy: function() {
		this.destroyExample();
		this.container.attr("class").remove().selectAll("*").remove();
		dhx.Dom.select("#haxe\\:trace").html().string("");
	}
...
d3.Moire.prototype = $extend(d3.Example.prototype,{
	runExample: function() {
		...
		svg.onNode("mousemove",function(n,i) {
			var mouse = dhx.Svg.mouse(n), r = (Math.sqrt(mouse[0]) + 10) / 10;
...
dhx.AccessDataTweenStyle.prototype = $extend(dhx.AccessTweenStyle.prototype,{
	...
	,colorTweenf: function(tween,priority) {
		...
		var name = this.name;
		var styleTween = function(d,i) {
			var f = tween(Reflect.field(d,"__dhx_data__"),i,thx.color.Colors.parse(window.getComputedStyle(d,null).getPropertyValue(name)));
			return function(t) {
				d.style.setProperty(name,f(t).toRgbString(),null == priority?"":priority);
			};
		};
...
d3.Stack.prototype = $extend(d3.Example.prototype,{
	...
	,transitionGroupEnd: function(d,i) {
		var t = dhx.Dom.selectNodeData(d).transition().duration(null,500);
	...
	,transitionStackEnd: function(d,i) {
		dhx.Dom.selectNode(d).transition().duration(null,500).attr("x")["float"](0).attr("width")["float"](d3.Stack.x({ x : .9, y : 0.0, y0 : 0.0}));
	}
	,description: function() {
		return "Stacked bars";
	}
...
if(!dhx.behavior) dhx.behavior = {}
dhx.behavior.ZoomEvent = $hxClasses["dhx.behavior.ZoomEvent"] = function(scale,tx,ty) {
	this.scale = scale;
	this.tx = tx;
	this.ty = ty;
};
dhx.behavior.ZoomEvent.__name__ = ["dhx","behavior","ZoomEvent"];
dhx.behavior.ZoomEvent.prototype = {
	scale: null
	,tx: null
	,ty: null
	,toString: function() {
	...
	,__class__: dhx.behavior.ZoomEvent
}
...
dhx.behavior.Zoom = $hxClasses["dhx.behavior.Zoom"] = function() {
	if(null == dhx.behavior.Zoom._outer) dhx.behavior.Zoom._outer = dhx.Dom.select("body").append("div").style("visibility").string("hidden").style("position").string("absolute").style("top").string("-3000px").style("height")["float"](0).style("overflow-y").string("scroll").append("div").style("height").string("2000px").node().parentNode;
};
dhx.behavior.Zoom.__name__ = ["dhx","behavior","Zoom"];
dhx.behavior.Zoom._outer = null;
dhx.behavior.Zoom.event = null;
dhx.behavior.Zoom.prototype = {
	_dispatcher: null
	,webkit533: null
	,_pan: null
	,_zoom: null
	...
	,mousedown: function(d,i) {
		this._pan = { x0 : this._x - dhx.Dom.event.clientX, y0 : this._y - dhx.Dom.event.clientY, target : d, data : Reflect.field(d,"__dhx_data__"), index : i};
		dhx.Dom.event.preventDefault();
		js.Lib.window.focus();
	}
	,mousemove: function(_,_1) {
		this._zoom = null;
		if(null != this._pan) {
			this._x = dhx.Dom.event.clientX + this._pan.x0;
			this._y = dhx.Dom.event.clientY + this._pan.y0;
	...
	,mousewheel: function(d,i) {
		var e = dhx.Dom.event;
		if(null == this._zoom) {
			var p = dhx.Svg.mouse(d.nearestViewportElement || d);
		...
		if("dblclick" == e.type) this._z = e.shiftKey?Math.ceil(this._z - 1):Math.floor(this._z + 1); else {
			...
			if(delta) {
				try {
					dhx.behavior.Zoom._outer.scrollTop = 1000;
					dhx.behavior.Zoom._outer.dispatchEvent(e);
					delta = 1000 - dhx.behavior.Zoom._outer.scrollTop;
				} catch( e1 ) {
				}
				delta *= .005;
			}
	...
	,dispatch: function(d,i) {
		if(null != this._dispatcher) {
			var event = new dhx.behavior.ZoomEvent(Math.pow(2,this._z),this._x,this._y);
			if(null != dhx.behavior.Zoom.event && event.scale == dhx.behavior.Zoom.event.scale && event.tx == dhx.behavior.Zoom.event.tx && event.ty == dhx.behavior.Zoom.event.ty) return;
			dhx.behavior.Zoom.event = event;
			try {
				this._dispatcher(d,i);
			} catch( e ) {
				haxe.Log.trace(e,{ fileName : "Zoom.hx", lineNumber : 141, className : "dhx.behavior.Zoom", methodName : "dispatch"});
			}
		}
	}
	,attach: function(dom,i) {
		var container = dhx.Dom.selectNode(dom);
		...
		dhx.Dom.selectNode(js.Lib.window).onNode("mousemove",this.mousemove.$bind(this)).onNode("mouseup",this.mouseup.$bind(this));
	}
	,zoom: function(f) {
		this._dispatcher = f;
		return this.attach.$bind(this);
	}
	,__class__: dhx.behavior.Zoom
}
...
d3.ZoomPan.prototype = $extend(d3.Example.prototype,{
	...
	,runExample: function() {
		...
		this.svg = this.container.append("svg:svg").attr("width")["float"](this.size[0] + this.padding[3] + this.padding[1]).attr("height")["float"](this.size[1] + this.padding[0] + this.padding[2]).attr("pointer-events").string("all").eachNode(function(n,i) {
			(new dhx.behavior.Zoom().zoom(me.redraw.$bind(me)))(n);
	...
	,redraw: function(_,_1) {
		if(null != dhx.behavior.Zoom.event) {
			var e = dhx.behavior.Zoom.event;
...
D3Examples.main = function() {
	...
	var classes = [d3.Area,d3.Bar,d3.CalendarVix,d3.Chord,d3.Moire,d3.NestedKey,d3.Pie,d3.PieAnimated,d3.Sort,d3.Stack,d3.Stream,d3.TransitionEquations,d3.CssTransition,d3.ZoomPan];
	dhx.Dom.select("#examples").selectAll("li").data(classes).enter().append("li").append("a").html().stringf(D3Examples.description).attr("href").string("#").on("click",D3Examples.click).first(function(d) {
		D3Examples.click(d,0);
	});
}
D3Examples.description = function(d,i) {
	return Type.getClassName(d).split(".").pop();
}
...
d3.CssTransition = $hxClasses["d3.CssTransition"] = function(cid) {
	d3.Example.call(this,cid);
};
d3.CssTransition.__name__ = ["d3","CssTransition"];
d3.CssTransition.__super__ = d3.Example;
d3.CssTransition.prototype = $extend(d3.Example.prototype,{
	transform: null
	,runExample: function() {
...
d3.TransitionEquations.prototype = $extend(d3.Example.prototype,{
	...
	,runTransition: function() {
		...
		this.container.selectAll("div.flying").eachNode(function(n,i) {
			var f = Reflect.field(n,"__dhx_data__").f;
			var t = dhx.Dom.selectNode(n).transition().delay(null,0).duration(null,2500).ease(thx.math.Ease.mode(ease,f)).style("left").string(d3.TransitionEquations.current % 2 == 0?width:"5px");
...
D3Examples.main()
//@ sourceMappingURL=d3examples.js.map